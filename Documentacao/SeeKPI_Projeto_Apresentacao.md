...
```mermaid
graph TD
    A["🔑 Admin"] --> B["📊 Visão Total + Gestão de Dados"]
    C["👔 Gerente"] --> D["📈 Dashboard de todos Supervisores"]
    E["👥 Supervisor"] --> F["📋 Dashboard da sua Equipe"]
    G["🧑‍💼 Vendedor"] --> H["📱 Seus KPIs Individuais"]
    
    A --> I["⬆️ Upload de Planilhas"]
    A --> J["👤 Gerenciar Usuários"]
    A --> K["🎯 Definir Metas"]
```
